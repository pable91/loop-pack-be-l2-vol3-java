...
[유저]     → POST /orders (Header: X-Entry-Token: {token})
           → 토큰 검증 → 주문 처리 → DEL entry-token:{userId}
```
...
토큰을 발급받고 주문하지 않으면 그 자리가 영영 비어있다. TTL이 만료되면 그 자리를 다음 유저에게 돌려줄 수 있다.

→ TTL 만료된 수만큼 다음 배치에서 추가 발급하는 로직이 필요하다.
...
시스템이 안정적으로 처리할 수 있는 TPS를 기준으로 스케줄러의 배치 크기를 결정한다.
...
[스케줄러] → 1초마다 175명 토큰 발급
         ...
         → DB 커넥션 175개 동시 점유 → 순간 부하 스파이크
```

**완화 전략:** 1초에 175명을 한 번에 발급하지 않고, 100ms마다 18명씩 나누어 발급한다.

```
AS-IS: 매 1초  → 200명 동시 발급
TO-BE: 매 100ms → ~20명씩 발급 → 부하가 10배 평탄화
```
